...
window.addEventListener("DOMContentLoaded", () => {
  const _fr = new EmojiRating("#emoji-rating");
});
...
class EmojiRating {
  ...
  update(e) {
    ...
    const percentRow = ((value - min) / (max - min)) * 100;
    const percent = percentRow.toFixed(2);
    ...
    const parts = ["right", "left", "low", "up"];

    parts.forEach((p) => {
      const el = this.face?.querySelector(`.fr-face-${p}`);
